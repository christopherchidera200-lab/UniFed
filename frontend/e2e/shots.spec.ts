...
// Design-review screenshots: render every surface (API mocked) and save PNGs.
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/v1/career/opportunities*", (r) =>
    r.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify([
      { id: "o1", title: "Backend Engineering Intern", company: "Naija Tech", employment_type: "internship", location: "Lagos", deadline: "2026-09-30" },
      { id: "o2", title: "Graduate Trainee, Data", company: "FedGov", employment_type: "full_time", location: "Abuja", deadline: "2026-10-15" }
    ]) }));
  ...
  await page.route("**/api/v1/academic/students/*/summary*", (r) =>
    r.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
      student_id: "s1", cgpa: 4.2, total_credit_units: 21, level: 300, published_grades: 7
    }) }));
});

async function shot(page: any, path: string, file: string) {
...
test("capture all pages", async ({ page }) => {
  // Log in once so authed surfaces render.
