...
RSpec.describe Identity::TokenService, type: :service do
  describe ".secret production fail-closed (vuln-0004)" do
    ...
    # Production is determined by RAILS_ENV/RACK_ENV (see TokenService.production?),
    # so we drive it through ENV rather than stubbing the memoized Rails.env.
    def with_env(env)
    ...
    it "raises in production when TOKEN_SERVICE_SECRET is absent" do
      with_env("RAILS_ENV" => "production", "TOKEN_SERVICE_SECRET" => nil, "OIDC_JWKS_PRIVATE" => rsa_key) do
    ...
    it "raises in production when TOKEN_SERVICE_SECRET EQUALS OIDC_JWKS_PRIVATE (same-value confusion)" do
      with_env("RAILS_ENV" => "production", "TOKEN_SERVICE_SECRET" => rsa_key, "OIDC_JWKS_PRIVATE" => rsa_key) do
        expect { Identity::TokenService.secret }
    ...
    it "uses TOKEN_SERVICE_SECRET when present and distinct (even in production)" do
      ...
      with_env("RAILS_ENV" => "production", "TOKEN_SERVICE_SECRET" => hmac, "OIDC_JWKS_PRIVATE" => rsa_key) do
