...
# Strong, NON-default secret for the test suite. Required now that
# Identity::TokenService.secret fails closed on empty/known-bad values
# (forgery-resistant auth). This is test-only and never shipped to staging/prod.
ENV["OIDC_JWKS_PRIVATE"] ||= "test-strong-secret-" + ("a" * 50)
